...
class RoleAgent(Agent):
    ...
    def potential_function(
        self, agent_pos: Tuple, ball_position: Tuple, role: int, strategy: int
    ):
    ...
    def role_assignment(self, teammates, roles, ball_position, strategy):
        ...
        for role in roles:
            ...
            for teammate_i in range(teammates_length):
                ...
                potential = self.potential_function(
                    teammate_pos, ball_position, role, strategy
                )
    ...
    def action(self) -> int:
        ...
        if self.curr_role is None or self.steps_counter % self.role_assign_period == 0:
            if self.my_team_has_ball(
                ...
                self.curr_role = role_assignments[
                    self.id if self.team == AGENT_TEAM else self.id - self.n_agents
                ]
                if self.curr_role == BALL_CARRIER:
                    ...
                    if self.player_nearby(
                        ...
                        agent_i = self.find_best_player(
                            agents_position
                            if self.team == AGENT_TEAM
                            else opponents_position,
                            opponents_position
                            if self.team == AGENT_TEAM
                            else agents_position,
                            my_position,
                            AGENT_TEAM if self.team == AGENT_TEAM else OPPONENT_TEAM,
                        )
                    ...
                    # is teammate
                    action = self.advance_to_pos(
                        my_position, ball_position, self.curr_role, ATTACKING
                    )
                ...
                action = self.advance_to_pos(
                    my_position, ball_position, self.curr_role, DEFENDING
                )
    ...
    def get_diamond_positions(self, ball_position: Tuple) -> List[Tuple]:
        ...
        supporter_left = (
            _validate_vertical_position(ball_x, -1),
            _validate_horizontal_position(ball_y, -1),
        )
        supporter_right = (
            _validate_vertical_position(ball_x, -1),
            _validate_horizontal_position(ball_y, 1),
        ...
        sub_supporter_left = (
            _validate_vertical_position(ball_x, -2),
            _validate_horizontal_position(ball_y, -2),
        )
        sub_supporter_right = (
            _validate_vertical_position(ball_x, -2),
            _validate_horizontal_position(ball_y, 2),
        ...
        winger_left = (
            _validate_vertical_position(ball_x, -3),
            _validate_horizontal_position(ball_y, -3),
        )
        winger_right = (
            _validate_vertical_position(ball_x, -3),
            _validate_horizontal_position(ball_y, 3),
    ...
    def advance_to_pos(
        self, my_position: Tuple, ball_position: Tuple, role: int, strategy: int
    ) -> Tuple:
        if strategy == ATTACKING:
            diamond_positions = self.get_diamond_positions(ball_position)
            ...
            if role <= 3:
                ...
                if self.team == AGENT_TEAM:
                    return self.go_toward_position(
                        my_position,
                        (ball_position[0] - DISTANCE_BETWEEN_LINES, ball_position[1]),
                    )
